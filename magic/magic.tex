...
\newcounter{definicija} % da bodo definicije oštevilčene
\newenvironment{definicija} 
{
	\stepcounter{definicija} % da se številčenje začne pri 1 in potem stopnjuje
	\begin{flushleft} % da bo besedilo definicije levo poravnano
	\textbf{Definicija \arabic{definicija}:} %kako naj izgleda beseda definicija, številčimo z arabskimi številkami
}
{
	\hfill $\square $ % da bo na koncu definicije kvadratek
	\end{flushleft}
}
% \cite za reference na literaturo
...
\begin{itemize}
\item
   \texttt{http://mathworld.wolfram.com/MagicSquare.html}
   \item
   \texttt{http://en.wikipedia.org/wiki/Magic\_square}
\end{itemize}
...
Primer magičnega kvadrata s stranico 3 je prikazan v tabeli \ref{3x3} na strani \pageref{3x3}.

\begin{table}[!h]
\centering
\caption{Magični kvadrat $3 \times 3$}
\vspace{2mm}
\label{3x3} % za sklicevanje
\begin{tabular}{|c|c|c|}
\hline
   8 & 1 & 6 \\ \hline % dva beksleša sta nova vrstica, nato nardi horizontalno črto
   3 & 5 & 7 \\ \hline
   4 & 9 & 2 \\ \hline
\end{tabular}
\end{table}
...
\vspace{2mm}
\label{loshu} % za sklicevanje
\begin{tabular}{|c|c|c|}
\hline
   4 & 9 & 2 \\ \hline % dva beksleša sta nova vrstica, nato nardi horizontalno črto
   3 & 5 & 7 \\ \hline
   8 & 1 & 6 \\ \hline
\end{tabular}
\end{table}
